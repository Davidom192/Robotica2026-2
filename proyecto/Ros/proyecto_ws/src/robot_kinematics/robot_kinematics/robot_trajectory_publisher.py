...
class PublicadorTrayectoria(Node): 
  ...
  def twist_callback(self, msg:Twist): 
    ...
    # Mandamos llamar TODAS las gráficas
    self.robot.imp_tray() 
    self.robot.imp_vel()   # <--- VELOCIDAD EF
    self.robot.imp_acc()   # <--- ACELERACIÓN EF
    self.robot.imp_junt() 
  ...
  def ps_callback(self, msg:PointStamped): 
    if self.is_moving or not hasattr(self, 'js_current'): 
      return 
    self.is_moving = True 

    # Obtener índices correctos 
    idx_sh = self.js_current.name.index("shoulder_joint") 
    idx_ar = self.js_current.name.index("arm_joint") 
    idx_fa = self.js_current.name.index("forearm_joint") 

    self.get_logger().info("Posición recibida: {}".format(str(msg.point))) 
    self.robot.def_tray( 
      th_i=(self.js_current.position[idx_sh], 
            self.js_current.position[idx_ar], 
            self.js_current.position[idx_fa]),  
      xi_f=(msg.point.x,  
            msg.point.y,  
            msg.point.z)) 
    ...
    # Descomentamos y agregamos las de velocidad y aceleración
    self.robot.imp_tray() 
    self.robot.imp_vel()   # <--- VELOCIDAD EF
    self.robot.imp_acc()   # <--- ACELERACIÓN EF
    self.robot.imp_junt() 
